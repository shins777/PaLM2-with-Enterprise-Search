...
tab1, tab2, tab3 = st.tabs(["Query execution", "Response Analysis", "Response Details "])

with tab1 : 
    # Get the user input
    user_input = get_text()

    if st.button("Ask to Palm2"):
...
with tab3:
    ...
    with st.expander("ES Snippets"):
        st.write(snippets_ctx)
    with st.expander("ES Segments"):
